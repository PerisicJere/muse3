...
def read_location_csv(file_path, dic_tuples_locations):
    with open(file_path, newline='', encoding="utf-8") as csvfile:
        ...
        for row in tqdm(csv_reader, desc="Processing Locations"):
            ...
            latitude = row[5]
            longitude = row[6]
            dic_tuples_locations[index] = (location_name, location_type, location_address, location_desc, location_img, latitude, longitude)
            index += 1
...
def insert_table(location_dir ,review_dir, user_dir, config_file, location_table, review_table, user_table):
    ...
    for csv_file in os.listdir(user_dir):
        ...
        for entry in tqdm(dic_tuples_locations, desc="Inserting Locations"):
            cursor_object.execute(
                "INSERT INTO MUSE_DB." + location_table + " (location_name, location_type, address, description, location_image, latitude, longitude)"
                                                          "values (%s, %s, %s, %s, %s, %s, %s)", (dic_tuples_locations[entry]))

    for entry in tqdm(dic_tuples_reviews, desc="Inserting Reviews"):
        cursor_object.execute(
            "INSERT INTO MUSE_DB." + review_table + " (user_id, location_id, stars, comment, review_image)"
                                                    "values (%s, %s, %s, %s, %s)", (dic_tuples_reviews[entry]))

    for entry in tqdm(dic_tuples_users, desc="Inserting Users"):
...
insert_table(path_to_location_dir, path_to_review_dir, path_to_user_dir, "connectorConfig.json", "artLocation",
             "review", "user")
